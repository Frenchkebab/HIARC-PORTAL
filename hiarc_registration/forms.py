...
class HiarcUserCreationForm(UserCreationForm):
    ...
    status = forms.CharField(widget=forms.Select(choices=STATUSES))
    semester = forms.CharField(widget=forms.Select(choices=SEMESTERS))
    major  = forms.CharField(widget=forms.Select(choices=MAJORS))
    phone_number = forms.CharField(max_length=20)
    motivation = forms.CharField(widget=forms.Textarea)
    portfolio = forms.CharField(required=False, widget=forms.Textarea)
